...
class MessageParserTorcs(IMessageParser.IMessageParser):
    def __init__(self):
        self.cs = None 
        self.msg = None
...
#         speed = cs.getSpeedX()/200      #max 200km/h
        ...
        #frontAndSpeed = numpy.array([self.cs.getTrack(9), self.cs.getSpeedX()]).astype(numpy.float)/200.0

        # HACK: Set front sensor to constant value
        frontAndSpeed = numpy.array([100.0, self.cs.getSpeedX()]).astype(numpy.float)/100.0
