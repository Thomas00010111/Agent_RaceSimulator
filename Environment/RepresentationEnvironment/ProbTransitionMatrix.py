...
#            self._transitionMatrix.fill(0)
        ...
        if filename:
            ...
            # probability to reach a stateRepresentation with a given action
            self._probabilityMatrix = SparseMatrix.SparseMatrix((noOfActions, noOfStates), dtype = float)
            self._probabilityMatrix.fill(0.0)
            
            # count transitions to stateRepresentation s' with action a
            self._transitionMatrix = SparseMatrix.SparseMatrix((noOfActions, noOfStates), dtype = int)
            self._transitionMatrix.fill(0)
    ...
    def isReliable(self, action, minNumExecutedActions=1):
        '''criteria to determine when state splitting can begin'''
        return numpy.sum(self._transitionMatrix[action]) > minNumExecutedActions  
...
#         return states.argmax()
    ...
    def CalcBestAction(self, actionValues):
        ''' based on the current N stateRepresentation and the probabilities between reachable N-1
        states calculate the best next N-1 stateRepresentation. Chose N-1 so that N will be best''' 
        assert(self.getNumberOfActions() == len(actionValues))
        print "CalcBestAction: self._probabilityMatrix: \n", self._probabilityMatrix
        print "CalcBestAction: stateValues: \n", actionValues
