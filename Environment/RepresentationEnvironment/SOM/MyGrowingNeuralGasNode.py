...
class MyGrowingNeuralGasNode(mdp.nodes.GrowingNeuralGasNode):
    ...
    **Attributes and methods of interest**
    ...
    def __init__(self, start_poss=None, eps_b=0.2, eps_n=0.006, max_age=50,
        ...
        #copy parameters
        myStart_poss = numpy.copy(start_poss)
        (self.eps_b, self.eps_n, self.max_age, self.lambda_, self.alpha,
         self.d, self.max_nodes) = (eps_b, eps_n, max_age, lambda_, alpha,
                                    d, max_nodes)

        super(MyGrowingNeuralGasNode, self).__init__(myStart_poss, eps_b, eps_n, max_age,
                 lambda_, alpha, d, max_nodes,
                 input_dim=None, dtype=None)

#         if start_poss is not None:
#             if self.dtype is None:
#                 self.dtype = start_poss[0].dtype
#             #node1 = self._add_node(_NGNodeData(start_poss[0], 0))
#             #node2 = self._add_node(_NGNodeData(start_poss[1], 1))
#             node1 = self._add_node(_NGNodeData(numpy.array([1, 0]), 0))
#             node2 = self._add_node(_NGNodeData(numpy.array([1, 0]), 1))
#             self._add_edge(node1, node2)

    

    def _add_node(self, pos):
        node = self.graph.add_node(_NGNodeData(pos, self.indexCounter))
        self.indexCounter += 1
        print "add from MyGrowingNeuralGas indexCounter: ", self.indexCounter
        return node
    
    def addNode(self, nodePosition):
        self.graph.add_node(_NGNodeData(nodePosition, self.indexCounter))
        self.indexCounter += 1 

#
#     # Fischle:  12 April 2015 added dome print statements to debug. Method same as original
#    # So why use this method? Commented out
...
#         # distance function
# #        print "_get_nearest_nodes: x:", x
# #        for node in self.graph.nodes:
# #            print "node.data.label: ", node.data.label
# #            print "node.data.pos: ", node.data.pos
#             
#         def _distance_from_node(node):
#             #return norm(node.data.pos-x)**2
#             tmp = node.data.pos - x
#             tmp = numpy.sqrt(sum(tmp * tmp))
#             
# #            resultMul = utils.mult(tmp, tmp)
# #            print "_get_nearest_nodes: x: ", x, "    tmp: ",tmp
# #            print "resultMul:", resultMul
#             return tmp
#         g = self.graph
